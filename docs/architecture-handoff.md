...
- 2026-06-22: Added experimental TinyFish browser-search support for car rentals through `/reservations/car-rentals/browser-search`, plus browser readiness at `/reservations/car-rentals/browser-readiness`. This is for fast public inventory discovery only; results are marked unverified and the agent still cannot reserve or pay.
- 2026-06-22: Added Browserless as the preferred next browser-search adapter for car rentals behind `BROWSERLESS_API_TOKEN`. Browserless runs a REST browser function before TinyFish, extracts visible price-like candidates from Kayak/Expedia/direct-brand pages, and still labels all results as unverified until reopened and confirmed.
